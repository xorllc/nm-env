...
public:
    ...
    {
        //cout<<hex<<setfill('0')<<setw(4)<<adr<<" "<<setw(2)<<(int)dat<<endl;
        switch(adr & 0xF003) {
    ...
    void audio(sound_info* info)
    {
        ...
        for(int i = 0; i < info->sample; i++) {
            ...
            while(!write_queue.empty() && cur >= write_queue.front().clk) {
                write_dat wd = write_queue.front();
                write_queue.pop();
                snd_write(wd.adr, wd.dat);
            }
            ...
            // サンプル(6ビット値)
            double d = (sq_produce(sq[0], sample_clk) + sq_produce(sq[1], sample_clk) + saw_produce(sample_clk)) / 32.0;
